...
async function loadStatsCount() {
  ...
  try {
    const topicsRes = await fetch(`${API}/admin/courses/${courseId}/topics`, {
      headers: { Authorization: "Bearer " + token }
    });
    ...
    for (const topic of topics) {
      const [qRes, aRes] = await Promise.all([
        ...
        fetch(`${API}/admin/topics/${topic.id}/assignments`, { headers: { Authorization: "Bearer " + token } })
      ]);
...
async function loadAllAssignments() {
  ...
  try {
    ...
    let allAssignments = [];
    for (const topic of topics) {
      const aRes = await fetch(`${API}/admin/topics/${topic.id}/assignments`, {
        headers: { Authorization: "Bearer " + token }
      });
      const assignments = await aRes.json();
