...
def download_audio(video_url: str):
    ...
    # Use a more unique name to avoid WinError 32 conflicts during rename
    file_prefix = f"ai_sum_{int(time.time())}_%(id)s"
    
    ydl_opts = {
        'format': 'bestaudio/best',
        'ffmpeg_location': BACKEND_DIR, # Point to local ffmpeg.exe
        'postprocessors': [{
    ...
    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
        ...
        audio_path = os.path.join(temp_dir, f"ai_sum_{int(time.time())}_{info['id']}.mp3")
        
        # Verify it exists, if not, try searching for it (yt-dlp might have used a slightly different name)
        if not os.path.exists(audio_path):
            # Fallback search in temp dir for this ID
    ...
    try:
        ...
        if video_url.startswith("uploads/"):
            ...
            if not os.path.exists(audio_file_path):
                raise HTTPException(status_code=404, detail=f"Local video file not found: {audio_file_path}")
