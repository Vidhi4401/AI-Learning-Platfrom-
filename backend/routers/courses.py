...
def delete_course(
    ...
    # 3. Delete related topics (and their sub-items should be handled by DB or manual cleanup)
    # Finding topics to clean up quizzes/assignments/videos
    topics = db.query(models.Topic).filter(models.Topic.course_id == course_id).all()
    topic_ids = [t.id for t in topics]
    
    if topic_ids:
        db.query(models.Quiz).filter(models.Quiz.topic_id.in_(topic_ids)).delete(synchronize_session=False)
        db.query(models.Assignment).filter(models.Assignment.topic_id.in_(topic_ids)).delete(synchronize_session=False)
        db.query(models.Video).filter(models.Video.topic_id.in_(topic_ids)).delete(synchronize_session=False)
        db.query(models.Topic).filter(models.Topic.id.in_(topic_ids)).delete(synchronize_session=False)
